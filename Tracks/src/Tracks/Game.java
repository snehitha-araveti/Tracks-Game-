...
public class Game {
    ...
    // puzzle generation algorithm to create solvable random puzzle
    //Time Complexity: O(w × h × maxTries)
    public boolean genPathAndSolution(int diffPercent) {
        ...
        //Generate random non-crossing path (DFS with backtracking)
       ...
       // Convert path to track pieces
        ...
        // giving clues and start and end nodes
    ...
    // Recursive Path Builder dfs with back tracking
    //Time Complexity: O(4^(w×h))
    ...
    // Create adjacency list representation of placed tracks
    ...
    // Algorithm 3: Path Finding - BFS Path Search
    // if valid path exists from start to end
    ...
    // check if path exits or not using bfs
    ...
    // use's  Greedy Heuristic - Work backwards from the end point
    // Time Complexity: O(w × h)
    // Not optimal: may take more moves than necessary
    // use's bfs , select minum node to construct the path
    ...
    // Distance Heuristic - Assign distance value to each cell from the END
    // use BFS on grid 
    // Time Complexity: O(w × h)
